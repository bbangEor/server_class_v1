...
public abstract class AbstractServer {
	...
	// 실행에 흐름이 필요하다 (순서 중요)
	public final void run() { // 메서드에서 final 사용시 자식클래스에서 사용불가능
		// 1. 서버 세팅 - 포트번호 할당
		try {
			setupServer();
			connection();
			setupStream();
			startService(); // -- 내부적으로 while 문 동작중 --
		} catch (IOException e) {
			e.printStackTrace();
		} finally {
			cleanUp();
	...
	private Thread createWriteThread() {
		return new Thread(() -> {
			try {
				String msg; // 서버 측 키보드에서 데이터를 한줄 라인으로 읽음
				while ((msg = keyboardReader.readLine()) != null) {
					// 클라이언트와 연결된 소켓에 데이터를 전송함
