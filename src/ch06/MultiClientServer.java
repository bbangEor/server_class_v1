...
public class MultiClientServer {
	...
	// 멀티스레드에서 사용할 수 있는 자료구조 -> 객체 배열 <- Vector<> : 멀티스레드에 안정적이다.
	...
	public static void main(String[] args) {
		...
		try (ServerSocket serverSocket = new ServerSocket(PORT)) {
			...
			while (true) {
				// 1. serverSocket.accept(); -> 호출하게되면 블록킹 상태가됨 (멈춰있음)
				// 2. 클라이언트가 연결을 요청하면 새로운 소켓 객체 생성이된다.
				// 3. 새로운 스레드를 만들어서 처리 (클라이언트가 데이터를 주고 받기 위한 스레드)
				// 4. 새로운 클라이언트가 접속하기까지 다시 대기 유지 (반복)
				
			Socket socket = serverSocket.accept();
			// 새로운 클라이언트가 연결되면 , 새로운 Thread 가 생성된다. 
